...
class Builder:
            ...
            for owner in owners:
                ...
                for branch in repo.get_branches():
                    if target_branch_name == branch.name:
                        logging.info(
                            f"Found branch {target_branch_name} for {dep['name']}"
                        )
    ...
    def create_fb_branch(self, branch_name, global_repo=False):
        ...
        for dep_name, url in found_branches.items():
            self.config_override["deps"].append(
                {"name": dep_name, "branch": branch_name, "url": url}
            )
        ...
        try:
            ...
            logging.warning(
                f"Failed to push {branch_name}. This could be due to no matching upstream branch."
            )
            logging.info(
        ...
        with open(GIT_SOURCES_FILE, "w+") as f:
            for dep in self.config["deps"]:
                ...
                if not repo_cloned():
                    ...
                    logging.info(
                        f"Files in the path for {dep['name']} is already exist"
                    )
    ...
    def check_deps(self):
        ...
        for dep in self.config_override["deps"]:
            if "url" in dep:
                ...
                target_url = next(
                    item for item in self.config["deps"] if item["name"] == dep["name"]
                )["url"]
    ...
    def validate_config(self):
        for dep in self.config["deps"]:
            if not os.path.abspath(dep["path"]).startswith(os.getcwd()):
                logging.error(
                    f"For dependency [{dep['name']} -> {os.path.abspath(dep['path'])}] "
...
def switch_branch(path, branch):
    ...
    try:
        cur_branch = (
            check_output("git symbolic-ref --short HEAD".split(), cwd=path)
            .decode()
            .strip()
        )
    except CalledProcessError:
        cur_branch = (
            check_output("git rev-parse HEAD".split(), cwd=path).decode().strip()
        )
    if cur_branch != branch:
        branches = check_output("git ls-remote --heads origin".split(), cwd=path)
        branches = [
            line.split("/")[-1] for line in branches.decode().strip().split("\n")
        ]
...
def main():
    ...
    parser.add_argument(
        ...
        "-g",
        dest="global_repo",
        help="find and use all branches with this name",
        action="store_true",
    )
    parser.add_argument(
        "--convert", help="convert .gitmodules config to yml", action="store_true"
    )
